...
require_once __DIR__ . '/queries/payments_list.php';
...
<head>
  ...
  <title>Платежи | Турагентство "Земеля"</title>
</head>
  ...
  <main>
    ...
    <?php if ($error): ?>
      <div class="mx-4 my-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg">
        Ошибка изменения платежа. Обратитесь к администратору
      </div>
    <?php endif; ?>
    <section class="min-h-screen bg-gray-100 py-10 px-4">

      <div class="bg-white rounded-2xl shadow-md p-6 mb-10 overflow-x-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Список платежей</h1>
        ...
        <table class="w-full border border-gray-200 rounded-xl overflow-hidden">
          <thead class="bg-gray-50">
            <tr class="text-left text-gray-700 font-medium">
              ...
              <th class="p-3 border-b">Номер договора</th>
              <th class="p-3 border-b">Дата</th>
              <th class="p-3 border-b">Сумма</th>
              <th class="p-3 border-b">Метод</th>
          ...
          <tbody class="text-gray-700">
            ...
            <?php foreach ($payments as $row): ?>
              <tr class="border-b">
                ...
                <td class="p-3"><?= $row['payment_date'] ?></td>
                <td class="p-3"><?= $row['amount'] ?> ₽</td>
                <td class="p-3"><?= $row['payment_method'] ?></td>


                <td class="p-3 flex gap-2">
                  <a href="/edits/payment-edit.php?id=<?= $row['payment_id'] ?>"
                    class="px-3 py-1 text-sm bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg">
                    Изменить
                  </a>

                  <a href="/edits/payment-delete.php?id=<?= $row['payment_id'] ?>"
                    class="px-3 py-1 text-sm bg-red-600 hover:bg-red-700 text-white rounded-lg"
                    onclick="return confirm('Удалить менеджера?')">
                    Удалить
                  </a>
                </td>
              </tr>
      ...
      <div class="bg-white rounded-2xl shadow-md p-8 w-full max-w-3xl mx-auto">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Добавить новый платеж</h2>

        <form id="adding-form" data-form="create_payment.php" class="grid grid-cols-1 md:grid-cols-2 gap-6">

          <!-- ID контракта -->
          <div>
            <label class="block mb-1 text-gray-700 font-medium">ID договора *</label>
            <input type="number" name="contract_id" required
              class="w-full border border-gray-300 rounded-xl py-2 px-3 focus:ring-2 focus:ring-blue-500" />
          ...
          <div class="md:col-span-2 flex justify-center mt-4">
            <button type="submit"
              class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-6 rounded-xl shadow">
              Добавить платеж
            </button>
          </div>
