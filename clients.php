<?php 
  ...
  require_once __DIR__ . '/queries/clients_list.php';
...
<head>
    ...
    <title>Клиенты | Турагентство "Земеля"</title>
</head>
...
<!-- Сообщения об успехе/ошибке -->
    ...
    <?php if ($error): ?>
      <div class="mx-4 my-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg">
        Ошибка изменения клиента. Обратитесь к администратору
      </div>
    <?php endif; ?>
...
<section class="min-h-screen bg-gray-100 py-10 px-4">
  ...
  <div class="bg-white rounded-2xl shadow-md p-6 mb-10 overflow-x-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Список клиентов</h1>
 ...
 <!-- Контейнер для сообщений -->
    ...
    <thead class="bg-gray-50">
        <tr class="text-left text-gray-700 font-medium">
            ...
            <th class="p-3 border-b">Фамилия</th>
            <th class="p-3 border-b">Имя</th>
            <th class="p-3 border-b">Телефон</th>
            <th class="p-3 border-b">Email</th>
            <th class="p-3 border-b">Паспорт</th>
            <th class="p-3 border-b">Тип клиента</th>
    ...
    <tbody class="text-gray-700">
        ...
        <tr class="border-b">
            <td class="p-3"><?= $row['client_id'] ?></td>
            <td class="p-3"><?= $row['surname'] ?></td>
            <td class="p-3"><?= $row['name'] ?></td>
            <td class="p-3">+<?= $row['phone'] ?></td>
            <td class="p-3"><?= $row['email'] ?></td>
            <td class="p-3"><?= $row['passport'] ?></td>
            <td class="p-3"><?= $row['client_type_name'] ?></td>

            <td class="p-3 flex gap-2">
                <a href="/edits/client-edit.php?id=<?= $row['client_id'] ?>" class="px-3 py-1 text-sm bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg">
                    Изменить
                </a>

                <a href="/edits/client-delete.php?id=<?= $row['client_id'] ?>" 
  ...
  <div class="bg-white rounded-2xl shadow-md p-8 w-full max-w-3xl mx-auto">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Добавить нового клиента</h2>

    <form id="adding-form" data-form="create_client.php"
