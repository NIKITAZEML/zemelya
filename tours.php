...
require_once __DIR__ . '/queries/tours_list.php';
...
<head>
  ...
  <title>Туры | Турагентство "Земеля"</title>
</head>
  ...
  <main>
    ...
    <section class="min-h-screen bg-gray-100 py-10 px-4">
      ...
      <div class="bg-white rounded-2xl shadow-md p-6 mb-10 overflow-x-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Список туров</h1>
        ...
        <table class="w-full border border-gray-200 rounded-xl overflow-hidden">
          <thead class="bg-gray-50">
            <tr class="text-left text-gray-700 font-medium">
              ...
              <th class="p-3 border-b">Название тура</th>
              <th class="p-3 border-b">Страна</th>
              <th class="p-3 border-b">Виза</th>
              <th class="p-3 border-b">Отель</th>
              <th class="p-3 border-b">Длительность</th>
              <th class="p-3 border-b">Цена</th>
              <th class="p-3 border-b">Описание</th>
          ...
          <tbody class="text-gray-700">
            ...
            <?php foreach ($tours as $row): ?>
              <tr class="border-b">
                <td class="p-3"><?= $row['tour_id'] ?></td>
                <td class="p-3"><?= $row['tour_name'] ?></td>
                <td class="p-3"><?= $row['country_name'] ?></td>
                <td class="p-3"><?= $row['visa_required'] ?></td>
                <td class="p-3">
                  <?= $row['hotel_name'] ?> (<?= $row['stars'] ?>★)
                </td>
                <td class="p-3"><?= $row['duration'] ?> дней</td>
                <td class="p-3"><?= $row['price'] ?> ₽</td>
                <td class="p-3"><?= $row['description'] ?></td>

                <td class="p-3 flex gap-2">
                  <a href="/edits/tour-edit.php?id=<?= $row['tour_id'] ?>"
                    class="px-3 py-1 text-sm bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg">
                    Изменить
                  </a>

                  <a href="/edits/tour-delete.php?id=<?= $row['tour_id'] ?>"
                    class="px-3 py-1 text-sm bg-red-600 hover:bg-red-700 text-white rounded-lg"
                    onclick="return confirm('Удалить тур?')">
                    Удалить
                  </a>
                </td>
              </tr>
      ...
      <div class="bg-white rounded-2xl shadow-md p-8 w-full max-w-3xl mx-auto">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Добавить новый тур</h2>

        <form id="adding-form" data-form="create_tour.php" class="grid grid-cols-1 md:grid-cols-2 gap-6">

          <!-- Название тура -->
          <div class="md:col-span-2">
            <label class="block mb-1 text-gray-700 font-medium">Название тура *</label>
            <input type="text" name="tour_name" required class="w-full border border-gray-300 rounded-xl py-2 px-3
          ...
          <div>
            ...
            <input type="number" name="country_id" required class="w-full border border-gray-300 rounded-xl py-2 px-3
                          focus:ring-2 focus:ring-blue-500 outline-none">
          </div>
          ...
          <div>
            ...
            <input type="number" name="hotel_id" required class="w-full border border-gray-300 rounded-xl py-2 px-3
                          focus:ring-2 focus:ring-blue-500 outline-none">
          </div>
          ...
          <div>
            ...
            <input type="number" name="duration" required min="1" class="w-full border border-gray-300 rounded-xl py-2 px-3
                          focus:ring-2 focus:ring-blue-500 outline-none">
          </div>
          ...
          <div>
            ...
            <input type="number" name="price" step="0.01" required class="w-full border border-gray-300 rounded-xl py-2 px-3
                          focus:ring-2 focus:ring-blue-500 outline-none">
          </div>
          ...
          <div class="md:col-span-2">
            ...
            <textarea name="description" rows="4" class="w-full border border-gray-300 rounded-xl py-2 px-3
